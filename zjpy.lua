...
sddz=addrjump(soo,0x12C08)+0x1c4
gg.addListItems({{address=sddz,flags=16,name='速度'}})

------------速度地址偏移---------

  jiasuaddr=sddz
  mrrw=sddz + 0x21C60C---原地任务>>> 210453397504;7521962645310668800;8389754676365911401;85899345920::最后一个偏移=1888✓  107,374,182,400;25D;4,294,967,296;1D::17从1D转找到有1~288D的1D地址
  ydrwddr=sddz+0x1E7E78-0xB0--原地任务获取
  ydrwcd=ydrwddr+0x8A8----原地任务修复 
  ydgypy = sddz + 0x1A6F7EC---原地光翼>>> ✓  :l_CandleSpace_0 往前两个
  julazuaddr=sddz+0x2079CEC----举蜡烛>>> ✓ CA内存
  yclzaddr = sddz+0x1AA09CC--隐藏蜡烛>>> ✓
  LightChipAddr = sddz+0x1FB81FC--吸光标>>> ✓
  csaddr = sddz+ 0x15B2FC8--0x15CBEBC--任意门>>> ✓
  huazb = sddz+0x1F428B0--花坐标>>> ✓  
  Azhahua = sddz + 0x226CA30--炸花>>> ✓  
  dianhuo = sddz + 0x15BB840---点蜡烛>>> ✓
  dtaddr = sddz + 0x20D6A80----地图判断>> ✓   
  ...
  rwxzaddr = sddz+0x46C700--人物旋转
    zldz = sddz-0x70--帧率地址
  hzdz = sddz-0x44--画质地址
  xjdiaoxiang= sddz+0x1A7061C--独狼献祭>>> ✓   
  luzhiyx = sddz + 0x20CFB20--录制影像>>> ✓
  
  ---------------人物地址偏移--------
  --[[  ]]
dengjiaddr = rwdz-0x22734--光翼数量>>> ✓
  hyxx = rwdz - 0x1F123C--好友信息>>> ✓  
  dz = rwdz - 0x3574--查询身高>>> ✓
  nengliang=rwdz+0x5210--无限能量>>> ✓
...
unemote = so + 10823528--全动作-1186976888改1384120352d xxxx
uncloset = so + 4053496 - 0xeb4--全衣柜446629856改1384120352d 10.19 可隐形斗
unnodes = so + 821420--好友树872415336改1384120352d 10.19
